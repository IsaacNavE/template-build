class AppLanguageSelector extends HTMLElement {
  connectedCallback() {
      ...
      this.innerHTML = `
          <div class="relative">
              <button id="language-btn" class="flex items-center space-x-2 text-white bg-black hover:bg-red-600 focus:outline-none py-2 px-4 rounded-full">
                  <img src="${langFlags[currentLang]}" alt="Flag" id="current-flag" class="w-6 h-6 transition-opacity duration-300" />
                  <span data-translate="language">Language</span>
                  <i class="fas fa-chevron-down"></i>
              </button>
              <div id="language-dropdown" class="hidden absolute right-0 mt-2 bg-black border border-red-600 rounded shadow-lg w-32 opacity-0 transform translate-y-4 transition-all duration-300">
                  <a href="#" onclick="changeLanguage('en')" class="block px-4 py-2 text-sm text-white hover:bg-red-600">
                      <img src="${langFlags.en}" alt="English Flag" class="w-6 h-6 inline mr-2" /> English
                  </a>
                  <a href="#" onclick="changeLanguage('es')" class="block px-4 py-2 text-sm text-white hover:bg-red-600">
                      <img src="${langFlags.es}" alt="Spanish Flag" class="w-6 h-6 inline mr-2" /> Español
                  </a>
                  <a href="#" onclick="changeLanguage('fr')" class="block px-4 py-2 text-sm text-white hover:bg-red-600">
